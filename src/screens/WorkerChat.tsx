  ...
  return (
    <div className="min-h-screen bg-chalk pb-28">
      <header className="sticky top-0 z-10 border-b border-line bg-surface px-4 py-3">
        ...
        <div className="nums mt-3 flex gap-4 text-xs">
          <span className="text-out">
            {t.entry.summary} {issued}
          </span>

          <span className="text-sabz">
            {t.entry.summaryReturned} {returned}
          </span>
            ...
            <div key={e.id}>
              ...
              <div
                className={`overflow-hidden rounded-xl ${
                  out
                    ? 'border-l-4 border-out bg-out-soft'
                    : 'border-r-4 border-sabz bg-sabz-soft'
                }`}
              >
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-line bg-surface p-4">
        <div className="flex gap-3">
          <button
            onClick={() => setSheet('issue')}
            className="flex-1 rounded-lg bg-out py-3.5 text-base font-semibold text-white"
          >
            {t.entry.issue}
          </button>
